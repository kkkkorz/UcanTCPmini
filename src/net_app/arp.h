...
void arp_init();
base_packet* arp_process(base_packet *packet);                                       // 解析为ARP数据包
void remove_header(packet *packet);                                     // 移除数据包头
base_packet* arp_reply(base_packet* data );                             // 回应ARP数据包
...
void arp_process_reply(base_packet *packet_receive);                         // 处理ARP应答
